...
public class StartAuditRound extends AbstractDoSDashboardEndpoint {
   ...
   * Class-wide logger
   */
  public static final Logger LOGGER = LogManager.getLogger(StartAuditRound.class);
  ...
  public void initializeAuditData(final DoSDashboard dosdb) {
    ...
    final List<ComparisonAudit> comparisonAudits =
        initializeAudits(contestResults, dosdb.auditInfo().riskLimit());
  ...
  public List<ContestResult> initializeContests(final Set<ContestToAudit> cta) {
    ...
    LOGGER
        .debug(String.format("[initializeContests: cta=%s, countedCRs=%s]", cta, countedCRs));
    return countedCRs;
  }
  ...
  public List<ComparisonAudit> initializeAudits(final List<ContestResult> contestResults,
    ...
    final List<ComparisonAudit> comparisonAudits =
        contestResults.stream().map(cr -> ComparisonAuditController.createAudit(cr, riskLimit))
            .collect(Collectors.toList());

    LOGGER
        .debug(String.format("[initializeAudits: contestResults=%s, " + "comparisonAudits=%s]",
  ...
  public void initializeCountyDashboard(final CountyDashboard cdb,
    ...
    final Set<String> drivingContestNames = comparisonAudits.stream()
        .filter(ca -> ca.contestResult()
            .getAuditReason() != AuditReason.OPPORTUNISTIC_BENEFITS)
        .map(ca -> ca.contestResult().getContestName()).collect(Collectors.toSet());
    ...
    if (cdb.getAudits().isEmpty()) {
      countyAudits = comparisonAudits.stream().filter(ca -> ca.isForCounty(cdb.county().id()))
    ...
    final AuditBoardDashboardASM auditDashboardASM =
        ASMUtilities.asmFor(AuditBoardDashboardASM.class, String.valueOf(cdb.id()));

    if (countyDashboardASM
        .currentState() != CountyDashboardState.BALLOT_MANIFEST_AND_CVRS_OK) {
    ...
    if (!countyDashboardASM.isInInitialState() && !countyDashboardASM.isInFinalState()) {
      LOGGER.debug(String
          .format("[initializeCountyDashboard: " + " cdb=%s, comparisonAudits=%s, " +
  ...
  public String endpointBody(final Request the_request, final Response the_response) {
    ...
    if (my_asm.get().currentState() == COMPLETE_AUDIT_INFO_SET) {
      ...
      // this needs to happen after uploading is done but before the audit is
      // started
  ...
  public Map<String, AuditReason> targetedContestReasons(final Set<ContestToAudit> ctas) {
    ...
    return contestToAudits.entrySet().stream().collect(Collectors
        .toMap((Map.Entry<String, List<ContestToAudit>> e) -> e.getKey(),
  ...
  public List<ContestResult> countAndSaveContests(final Set<ContestToAudit> cta) {
    ...
    return ContestCounter.countAllContests(true).stream().map(cr -> {
      ...
      return cr;
    }).map(Persistence::persist).collect(Collectors.toList());
  }
  ...
  public List<Selection> makeSelections(final List<ComparisonAudit> comparisonAudits,
                                        final String seed, final BigDecimal riskLimit) {
    ...
    for (final ComparisonAudit comparisonAudit : comparisonAudits) {
      ...
      if (contestResult.getAuditReason().isTargeted()) {
        ...
        LOGGER.info(String.format("[makeSelections for ContestResult: contestName=%s, " +
                                  "contestResult.contestCVRIds=%s, selection=%s, " +
                                  "selection.contestCVRIds=%s, startIndex=%d, endIndex=%d]",
                                  contestResult.getContestName(),
                                  comparisonAudit.getContestCVRIds(), selection,
  ...
  public String startRound(final Request the_request, final Response the_response) {
    ...
    List<ComparisonAudit> comparisonAudits = Persistence.getAll(ComparisonAudit.class).stream()
        .filter(ca -> ca.isTargeted() && !ca.isFinished()).collect(Collectors.toList());
    ...
    List<Selection> selections = makeSelections(comparisonAudits, seed, riskLimit);
      ...
      for (final CountyDashboard cdb : dashboardsToStart()) {
        try {
          ...
          final List<CastVoteRecord> ballotSequenceCVRs =
              PhantomBallots.removePhantomRecords(PhantomBallots
                  .auditPhantomRecords(cdb, segment.cvrsInBallotSequence()));

          // ballotSequence is *just* the CVR IDs, as expected.
          final List<Long> ballotSequence =
              ballotSequenceCVRs.stream().map(cvr -> cvr.id()).collect(Collectors.toList());

          // similar message also sent to info below, this could be a big line
          LOGGER.trace(String
              .format("[startRound:" + " county=%s, round=%s, segment.auditSequence()=%s," +
                      " segment.ballotSequence()=%s, cdb.comparisonAudits=%s,", cdb.county(),
                      cdb.currentRound(), segment.auditSequence(), ballotSequence,
          ...
          if (ballotSequence.isEmpty()) {
            LOGGER.info(String
                .format("[startRound: no ballots to audit in %s County, skipping round]",
                        cdb.county()));
          ...
          ComparisonAuditController.startRound(cdb, cdb.comparisonAudits(),
                                               segment.auditSequence(), ballotSequence);
          Persistence.saveOrUpdate(cdb);

          LOGGER.info(String.format("[startRound: Round %d for %s County started normally." +
          ...
          final String msg =
              String.format("could not start round for %s County", cdb.county().name());
  ...
  public List<CountyDashboard> dashboardsToStart() {
    ...
    final List<CountyDashboard> result =
        cdbs.stream().filter(cdb -> isReadyToStartAudit(cdb)).collect(Collectors.toList());
